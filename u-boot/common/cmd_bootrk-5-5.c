...
int do_bootrk(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
   ...
   printf("00kernel   @ 0x%08x (0x%08x)\n", hdr->kernel_addr, hdr->kernel_size);
   printf("00ramdisk  @ 0x%08x (0x%08x)\n", hdr->ramdisk_addr, hdr->ramdisk_size);
#ifdef CONFIG_UBOOT_CHARGE
	...
	if (ptn) {
		...
		hdr = rk_load_image_from_storage(ptn, &images);
		 printf("01kernel   @ 0x%08x (0x%08x)\n", hdr->kernel_addr, hdr->kernel_size);
		 printf("01ramdisk  @ 0x%08x (0x%08x)\n", hdr->ramdisk_addr, hdr->ramdisk_size);
		if (hdr == NULL) {
			goto fail;
		}
	} else {
		...
		printf("02kernel   @ 0x%08x (0x%08x)\n", hdr->kernel_addr, hdr->kernel_size);
		printf("02ramdisk  @ 0x%08x (0x%08x)\n", hdr->ramdisk_addr, hdr->ramdisk_size);
		if (hdr == NULL) {
			goto fail;
		}
	}
...
#endif
    printf("03kernel   @ 0x%08x (0x%08x)\n", hdr->kernel_addr, hdr->kernel_size);
	printf("03ramdisk  @ 0x%08x (0x%08x)\n", hdr->ramdisk_addr, hdr->ramdisk_size);
	rk_commandline_setenv(boot_source, hdr, charge);
	printf("04kernel   @ 0x%08x (0x%08x)\n", hdr->kernel_addr, hdr->kernel_size);
	printf("04ramdisk  @ 0x%08x (0x%08x)\n", hdr->ramdisk_addr, hdr->ramdisk_size);
	...
	rk_module_deinit();
    printf("05kernel   @ 0x%08x (0x%08x)\n", hdr->kernel_addr, hdr->kernel_size);
	printf("05ramdisk  @ 0x%08x (0x%08x)\n", hdr->ramdisk_addr, hdr->ramdisk_size);
